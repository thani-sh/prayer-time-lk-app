package me.thanish.prayers.times
...
fun getPrayerTimesForDate(context: Context, city: String, date: LocalDate): PrayerTimes {
    val methodology = Preferences.getMethodology()
    val times = readTimes(context, city)[date.monthValue - 1][date.dayOfMonth - 1]
    val asrIndex = if (methodology == "Shafi") 3 else 4
    return PrayerTimes(
        city,
        date,
        ...
        toPrayerTime(city, date, name = "asr", times[asrIndex]),
        toPrayerTime(city, date, name = "maghrib", times[5]),
        toPrayerTime(city, date, name = "isha", times[6]),
    )
}
